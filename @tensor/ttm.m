...
% $Id: ttm.m,v 1.18 2010/03/19 23:46:31 tgkolda Exp $

% Fast ttm, 2012/07/17, Phan Anh Huy
...
if issparse(V) && isa(X.data,'single')
    V = full(V);
end

if n == 1
    newdata = reshape(X.data,sz(n),[]);
    newdata = V*newdata;
elseif n == N
    newdata = reshape(X.data,[],sz(n));
    newdata = newdata * V.';
    ...
    newdata = X.data;
    if N > 3
        newdata = reshape(newdata,prod(sz(1:n-1)),sz(n),[]);
    end
    newdata = permute(newdata,[2 1 3]);
    newdata = V * reshape(newdata,sz(n),[]); % error when V: sparse ans newdata single
    newdata = reshape(newdata,[],prod(sz(1:n-1)),prod(sz(n+1:end)));
    newdata = ipermute(newdata,[2 1 3]);
    
    %newdata = reshape(X.data,prod(sz(1:n-1)),sz(n),1,[]);
    %newdata = bsxfun(@times,newdata,reshape(V,1,sz(n),size(V,2)));
    %newdata = sum(newdata,n);
end
sz(n) = size(V,1);
Y = tensor(newdata,sz);
%Y = ipermute(Y,order);

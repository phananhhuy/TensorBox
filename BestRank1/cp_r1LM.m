function [P,output] = cp_r1LM(X,R,opts)
% fLMa - Fast Damped Gauss-Newton (Levenberg-Marquard) algorithm for best
...
% lda = ttv(X,U)/prod(cellfun(@(x) x'*x, Uinit,'uni',1));
% U = cellfun(@(x) x*lda^(1/N),U,'uni',0);
...
if isempty(param.mu) || (param.mu < 0)
    mu=param.tau*max(mm(:));    % initialize damping mu, nu
else
    mu = param.mu;
end
...
if param.TraceRank1Norm
    Rank1Norm = zeros(param.maxiters,1);
    Rank1Norm(1) = norm(prod(C(bsxfun(@plus,(1:R+1:R^2)',(0:N-1)*R^2)),2));
end
...
%         mu = 0;
        [U, mu, g,d] = lmrank1_update(X,U,mu,mu_opt); % update U nr6
...
%         [U, d, g] = update_nr6(X,U,mu,Gxn,C,Gamman); % fast update U
    ...
    if (err2>err)|| isnan(err2)                           % The step is not accepted
        ...
        if (norm(d)> param.tol) ... %&& (sqrt(err2-err)>(1e-5*normX)) ...
                && (mu <= param.max_mu) && (mu_inc_count <= param.max_mu_inc_count)
    ...
    if param.TraceRank1Norm
        lambdanorm = sum(prod(C(bsxfun(@plus,(1:R+1:R^2)',(0:N-1)*R^2)),2));
        Rank1Norm(iter) = sqrt(lambdanorm);
        if lambdanorm > param.max_rank1norm
            break
        end
    end
...
if nargout >=2
    ...
    if param.TraceRank1Norm
        output.Rank1Norm = Rank1Norm(1:iter);
    end
end
...
param.addOptional('maxboost',0);
param.addOptional('fullupdate',false);
param.addOptional('mu',[]);
param.addOptional('max_mu',1e10);
param.addOptional('max_mu_inc_count',20); % maximum of times to increase the damped parameter
param.addParamValue('MaxRecursivelevel',0);

...
% Xnt = sptenmat(X,n,'t'); 
N = ndims(X);
X = permute(X,[1:n-1 n+1:N n]);
siz = size(X);
X = reshape(X,[prod(siz(1:end-1)) siz(end)]);
X = spmatrix(X);
...
% dense matrix 
Y = X * V'; % modified on April 7, 2013

% modified on April 7, 2013
if nnz(Y) <= 0.5 * prod(siz)
    % Final result is a *sparse* tensor
    %Ynt = sptenmat(Z, rdims, cdims, siz);
    %Y = sptensor(Ynt);
    Y = sparse(Y);
    [ir,ic,val] = find(Y);
    Y = sptensor([ir,ic],val,size(Y));
    Y = reshape(Y,siz);
else

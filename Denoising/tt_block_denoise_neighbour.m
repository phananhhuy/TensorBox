...
sub_Y = Y(curr_xy(1):botright(1),curr_xy(2):botright(2),:);
...
if options.get_rank
    ...
    if ~isempty(param.process_id)
        file_blkrank = sprintf('%s_denoise_blkrank_%s_%d.txt',param.process_name,decomposition_method,param.process_id);
        fid = fopen(file_blkrank,'a+');
        fprintf(fid,'%d,%d, %d, %s\n',curr_xy(1),curr_xy(2),param.process_id,sprintf('%d, ',apx_rank));
    
    else
        file_blkrank = sprintf('%s_denoise_blkrank.txt',param.process_name);
        fid = fopen(file_blkrank,'a+');
...
% if options.get_rank
%     rec_block = struct('data',rec_block,'rank',apx_rank);
% end
...
%                 C = 1.05; % might be high if noise 0 dB
                ...
                accuracy = 1.5*C^2*noise_level^2*numel(data_sel);

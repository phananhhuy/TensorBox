...
% Initialize the residue tensor 
%
if isempty(param.residue_estimate_tensor)% = x(k) - x(k-1)
    eb = err/(mu + R);
else
    eb = (err - param.residue_estimate_tensor(:))/(mu + R);
end
curr_norm = norm(err);
...
param.addOptional('residue_estimate_tensor',[]);

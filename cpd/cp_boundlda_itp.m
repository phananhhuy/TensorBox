function [P,output] = cp_boundlda_itp(X,R,delta,opts)
%
%  min     |X - P|_F^2
%  subjec to  sum_n lambda_n^2 < delta
%
...
% v = x;
    ...
    U = vec2mat(v);
    U = normalizeU(U);
    [D,Z1,K,Hc] = cp_hessian_rank1norm(U); % Hessian of the constraint function |lambda|^2
    [G,Z,K0,V,F] = cp_hessian_inn(U);% Hessian of the objective function 1/2|Y-X|^2
    Ho = G + Z*K0*Z' + V*F*V';
    norm(H-Ho-Hc,'fro')
    ...
    U = vec2mat(x);
    U = normalizeU(U);
    [D,Z1,K,Hc] = cp_hessian_rank1norm(U);
    [G,Z,K0,V,F] = cp_hessian_inn(U);
    Ho = G + Z*K0*Z' + V*F*V';
     norm(H - Hc-Ho,'fro')
...
%         end
    ...
    %%
    function [D,Z1,K,H] = cp_hessian_rank1norm(U)
        % Hessian of the function f = 1/2*norm(\lambda)^2
        % Ho = D + 2 * Z1 * K * Z1'
        %         U = normalizeU(U);
...
%     end
    ...
    function H = compact_hessian_fc(x,lambda)
        ...
        %[G,Z,Kc,V,F,Kcm,Gamma,Gammas] = cp_hessian_inn(U);
...
%%
    ...
    function [G,Z,K0,V,F] = cp_hessian_inn(U)
        % Hessian of the function f = 1/2*|Y - Y_hat|_F^2
        % H = G + Z* K * Z' + V* F * V';
        %
        Prr = per_vectrans(R,R);
        Gamma_n = zeros(R,R,N);
...
%%
  ...
  function Hv = hessianmultiply_fc(x,lambda,v)
        ...
        %[D2,Z2,K2,Ho2] = cp_hessian_rank1norm(U);%norm(Hv - Ho2*v) % checked
        
        % compute Hessian time v for the objective function
        % 1/2 * norm(Y-Yhat,'fro')^2
        % [G,Z,K0,Vh,F] = cp_hessian_inn(U);
        % Hc = G + Z*K0*Z' + Vh*F*Vh';

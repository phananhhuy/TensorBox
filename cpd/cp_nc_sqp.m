function [P,output] = cp_nc_sqp(X,R,delta,opts)
...
Opts.foptions(2) = 1e-5; %Termination tolerance for X
Opts.foptions(3) = 1e-4; %-Termination tolerance on F.(Default: 1e-4).
Opts.foptions(14) =  min(sum(I)*R*100,param.maxiters);
...
output = history;
    ...
    function [f,fc] = fobjconst(x)
        ...
        % constraint values
        P = ktensor(U);
        fc = (normX^2 + norm(P)^2 - 2* innerprod(X,P) - delta^2)/2;
    ...
    function H = hessian_ff(x,lambda)
        ...
        if 0
            ...
            for n = 1:N
                ...
                Gamma_n_lda = diag(beta_n) + lambda(1) * Gamma_n;
...
%%
    function H = compact_hessian_fc(x,lambda)
        ...
        V = [];
        for n = 1:N
            temp = bsxfun(@rdivide,kron(eye(R),U{n}),reshape(Gamma_n(:,:,n)+1e-8,1,[]));
            V = [V;temp];
        end
...
%         P = ktensor(U);
%
%         cineq = (normX^2 + norm(P)^2 - 2* innerprod(X,P) - delta^2)/2;
%         ceq = [];
%
%         gradceq = [];gradineq=[];
%         if nargout >2

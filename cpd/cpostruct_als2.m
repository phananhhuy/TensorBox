function [P,output] = cp_fastals_orth(X,R,opts)
...
for iter = 1:param.maxiters
    ...
    %         P = ktensor(lambda,U);
    %         if ~param.TraceFit
    %             param_ls.mod_err = norm(P)^2 - 2 * innerprod(X,P);
    %         else
    ...
    lambda = (sum(U{nonorthomodes(end)}.^2));
    %CPOALS1
    for n = nonorthomodes
        G = mttkrp(X,U,n);
        if n~=nonorthomodes(end)
            U{n} = G*diag(1./lambda);
            U{n} = bsxfun(@rdivide,U{n},sqrt(sum(U{n}.^2)));
        else
            U{n} = G;
        end
...
%     % CP-ALS-2
%     Z = khatrirao(U(param.orthomodes));
%     G = Xm * Z;
%     %optsrank1 = cp_fastals;
%     if numel(nonorthomodes) == 1
%         U{nonorthomodes(1)} = G;
%     else
%         for r = 1:R
%             if numel(nonorthomodes)>2
%                 optsrank1.init = 'nvec';
%                 optsrank1.maxiters = 1000;
%                 optsrank1.printit = 1;
%                 %Pr1 = cp_fastals(tensor(reshape(G(:,r),I(nonorthomodes))),1,optsrank1);
%                 Pr1 = mtucker_als(tensor(reshape(G(:,r),I(nonorthomodes))),ones(1,numel(nonorthomodes)),optsrank1);
%                 for kn = 1:numel(nonorthomodes)
%                     U{nonorthomodes(kn)}(:,r) = Pr1.U{kn};
%                 end
%                 U{nonorthomodes(end)}(:,r) = U{nonorthomodes(end)}(:,r) * Pr1.lambda;
%             elseif numel(nonorthomodes)==2
%                 Gr = reshape(G(:,r),I(nonorthomodes));
%                 [un,sn,vn] = svd(Gr,0);
%                 [sn,maxid] = max(diag(sn));
%                 
%                 U{nonorthomodes(1)}(:,r) = un(:,maxid(1));
%                 U{nonorthomodes(2)}(:,r) = vn(:,maxid(1))*sn;
%                 
% %                 [un,sn,vn] = svds(Gr,1,'L');
% %                 U{nonorthomodes(1)}(:,r) = un;
% %                 U{nonorthomodes(2)}(:,r) = vn*sn;
%             end
%         end
%     end
...
param.addOptional('init','random',@(x) (iscell(x) || isa(x,'ktensor')||ismember(x(1:4),{'rand' 'nvec' 'fibe' 'orth'})));
